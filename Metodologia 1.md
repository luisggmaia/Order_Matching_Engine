...
1. `struct Order`
    -
    { `Type type`, `Side side`, `int qty`, `float price`, `int time` }
...
1. *id*
    -
    `"{type}_{side}_{time}_{price}%.2f"`

    `type`: `Type::Limit` (*limit* ou *market*) ou `Type::Peg` (*pegged*)

    `side`: `Side::Buy` ou `Side::Sell`

    `time`:

    `price`: vazio, se tipo *pegged*.

    Pois é o que basta para localizar uma ordem. O `type` (saber ser *pegged* ou não); o `side`; o `time`, para a ordem de prioridade; e o `price`, para localizar no `map` de prioridade, em caso de ordens *limit*.
