...
int OrderBook::insert_order(Order order) {
    ...
    switch (order.type) {
        ...
        case Type::Limit: {
            ...
            /*
            if ((order.side == Side::Buy && order.price >= limit_map(Side::Sell).begin()->first)
                || (order.side == Side::Sell && order.price <= limit_map(Side::Buy).begin()->first))
                update_book();
            */
            update_book();
            break;
        ...
        case Type::Market: {
            ...
            order.price = limit_map(opposite(order.side)).begin()->first;
...
bool OrderBook::change_order(int order_id, int qty, std::optional<int> opt_price) {
    ...
    if (it->type == Type::Peg) {
        ...
        if (opt_price.has_value() && opt_price.value() != it->price) {
            ...
            new_it_map = limit_map(it->side).try_emplace(opt_price.value).first;
        }
        ...
        /*
        if ((it->side == Side::Buy && price >= limit_map(Side::Sell).begin()->first)
            || (it->side == Side::Sell && price <= limit_map(Side::Buy).begin()->first))
            update_book();
        */
        update_book();
    }
...
void OrderBook::match(Order& bid_order, Order& offer_order) {
    ...
    trade_.price = (bid_order.seq < offer_order.seq) ? state(Side::Buy).book_it->first : state(Side::Sell).book_it->first;
    ...
    if (bid_order.qty < offer_order.qty) {
        ...
        if (ids.contains(bid_order.id)) {
            ...
            history.splice(history.begin(), *state(Side::Buy).list_ptr, ids[bid_order.id]);
        ...
        if (ids.contains(offer_order.id)) {
            ...
            history.splice(history.begin(), *state(Side::Sell).list_ptr, ids[offer_order.id]);
