...
class OrderBook {
    ...
    private:
        ...
        // 20 Bytes (in a 64-bit machine; 12 Bytes in a 32-bit).
        struct TopOrder {
        ...
        // 20 Bytes (in a 64-bit machine; 12 Bytes in a 32-bit).
        struct TopQueue {
            OrderQueue *lim_queue = nullptr, *peg_queue = nullptr;
            int price;
        };
        ...
        TopQueue top_queues_[2] = {{.peg_queue = &peg_book_[0]}, {.peg_queue = &peg_book_[1]}} ;
...
bool OrderBook::resolve_top_queue(Side s) noexcept {
    ...
    t_q.lim_queue = &top_map.begin()->second;
    t_q.price = price_of(s, top_map.begin()->first);
    return true;
}
...
bool OrderBook::resolve_top_order(Side s) noexcept {
    ...
    TopQueue& t_q = top_queue(s);
    if (t_q.peg_queue->empty() && t_q.lim_queue->empty())
        return false;

    TopOrder& t = top_order(s);
    t.is_peg = t_q.lim_queue->empty() || (!t_q.peg_queue->empty() && t_q.peg_queue->front().seq < t_q.lim_queue->front().seq);
    t.queue = t.is_peg ? t_q.peg_queue : t_q.lim_queue;
...
void OrderBook::run_matching() {
    ...
    while (resolve_top_queue(Side::Buy) && resolve_top_queue(Side::Sell)) {
        ...
        if (buy_t_q.lim_queue->empty())
            lim_map(Side::Buy).erase(-buy_t_q.price); // key_of(Side::Buy, buy_t_q.price)
        if (sell_t_q.lim_queue->empty())
            lim_map(Side::Sell).erase(sell_t_q.price); // key_of(Side::Sell, sell_t_q.price)
    }
}
